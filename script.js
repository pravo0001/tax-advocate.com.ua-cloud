...
(() => {
  const feedList = document.querySelector("#blog-feed-list");
  const topicFilter = document.querySelector("#blog-topic-filter");
  const rangeEl = document.querySelector("#blog-visible-range");
  if (!(feedList instanceof HTMLElement) || !(topicFilter instanceof HTMLSelectElement) || !(rangeEl instanceof HTMLElement)) {
    return;
  }

  const items = [...feedList.querySelectorAll(".blog-feed-item")];
  ...
  const syncFooter = () => {
    ...
    if (visibleCount === 0) {
      rangeEl.textContent = "Нічого не знайдено — спробуйте інший фільтр";
      return;
    }

    if (value === "all" && visibleCount === total) {
      rangeEl.textContent = `1–${total} з ${total}`;
      return;
    }

    rangeEl.textContent = `Знайдено: ${visibleCount} з ${total}`;
  };

  topicFilter.addEventListener("change", syncFooter);
  syncFooter();
})();
